...
```Dba```底层使用的是 ```JdbcTemplate``` 或者 ```NamedJdbcTemplate```，用法与它们类似，只是把sql语句和参数封装成了```Sql```或者```NamedSql```
...
对象属性、条件参数支持直接使用枚举，默认存到数据库中的是枚举的 ordinal 值。 如果要保存任意设定的字符串值，请让这个枚举实现```CodeEnum```接口。
