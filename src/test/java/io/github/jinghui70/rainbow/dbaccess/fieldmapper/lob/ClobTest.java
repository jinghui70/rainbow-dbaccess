...
import io.github.jinghui70.rainbow.dbaccess.fieldmapper.ObjectFieldMapper;
...
public class ClobTest extends LobTest {
    ...
    public void testObject() {
        ...
        JSONObject jsonObject = sql.queryForValue(ObjectFieldMapper.of(JSONObject.class));
        assertEquals("Tom", jsonObject.getStr("name"));

        SimpleObject tom = sql.queryForValue(ObjectFieldMapper.of(SimpleObject.class));
        assertTom(tom);
    }
    ...
    public void testArray() {
        ...
        array = sql.queryForValue(ObjectFieldMapper.ofArray(SimpleObject.class));
        assertTom(array[0]);
        assertJerry(array[1]);

        JSONArray jsonArray = sql.queryForValue(ObjectFieldMapper.of(JSONArray.class));
    ...
    public void testList() {
        ...
        Sql sql = dba.select("LOB_ARRAY").from("LOB_OBJECT").where("id", 1);
        list = sql.queryForValue(ObjectFieldMapper.ofList(SimpleObject.class));
        assertTom(list.get(0));
        assertJerry(list.get(1));
    }
    ...
    public void testMap() {
        ...
        FieldMapper<Map<String, List<SimpleObject>>> mapMapper = new ObjectFieldMapper<>(
                new TypeReference<>() {
                });
        ...
        Map<String, Object> simpleMap = sql.queryForValue(ObjectFieldMapper.ofMap());
