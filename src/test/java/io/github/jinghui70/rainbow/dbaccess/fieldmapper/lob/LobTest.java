...
public abstract class LobTest {
    ...
    public static final String contentStr = "在不远的未来，人类终于解锁了时间旅行的秘密。这一发现立即引起了全球的轰动，科学家们、历史学家们、甚至普通大众都对能够亲眼目睹历史或探索未来的可能性感到兴奋不已。然而，随着技术的发展，政府和国际组织很快意识到，时间旅行如果不受控制，可能会对现实造成不可逆转的影响。" +
    ...
    public static final byte[] contentByte = contentStr.getBytes(StandardCharsets.UTF_8);

    public static final List<SimpleObject> objectList = ListUtil.of(
            new SimpleObject(10, "Tom", 100.0),
            new SimpleObject(20, "Jerry", 60.0)
    );

    public void assertTom(SimpleObject so) {
        assertEquals(10, so.getId());
        assertEquals("Tom", so.getName());
        assertEquals(100, so.getScore());
    }

    public void assertJerry(SimpleObject so) {
