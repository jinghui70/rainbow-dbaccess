...
public class BlobTest extends BaseTest {
    ...
    @BeforeEach
    void init() {
        dba.sql("DROP TABLE IF EXISTS BLOB_OBJECT").execute();
        dba.createTable("BLOB_OBJECT",
    ...
    public void test() {
        ...
        list = set.stream().sorted(Comparator.comparing(SimpleObject::getId)).toList();
        assertTom(list.get(0));
        assertJerry(list.get(1));
        ...
        // JSONObject
        JSONObject jsonObject = dba.select("LOB_OBJECT")
                .from("BLOB_OBJECT").where("id", 1)
                .queryForValue(new BlobObjectFieldMapper<>(JSONObject.class));
        assertEquals("Tom", jsonObject.getStr("name"));

        // 测试提取 JsonArray
        JSONArray jsonArray = dba.select("LOB_ARRAY")
                .from("BLOB_OBJECT").where("id", 1)
                .queryForValue(new BlobObjectFieldMapper<>(JSONArray.class));
